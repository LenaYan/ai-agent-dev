...
跑法（需要 ANTHROPIC_API_KEY）：
    uv run python scripts/eval_judge.py
    uv run python scripts/eval_judge.py --model claude-sonnet-5   # 升级对比
"""
...
def main() -> int:
    ...
    if not os.environ.get("ANTHROPIC_API_KEY"):
        print("✗ 需要 ANTHROPIC_API_KEY（写进 .env 或 export）才能接真 LLM。", file=sys.stderr)
        return 2
