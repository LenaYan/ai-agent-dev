...
- **⑦ 分层那道边界的最终验收**：回到不带 extra 的环境（`uv sync` 卸掉 `sentence-transformers` 及其 28 个传递依赖，`.venv` 897M→82M），**393 个测试一个不掉**。领域层全靠假 embedder 跑 —— `Embedder` 协议这道边界唯一要证明的事，证明了。
...
- **下一步**：
  ...
  2. 若还要推进向量路线，**下一个受控变量是 hybrid（dense + 字面）而不是换模型** —— 因为两者失败模式**不重合**（向量丢的 `place_value_ignored` 字面稳稳命中），融合有理论收益。但守同一条纪律：先在同工作点上量，再决定要不要留。
