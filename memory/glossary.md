...
- **RSI（递归自我改进，recursive self-improvement）** — AI 承担原本由研究员完成的工作（写训练代码、设计实验、筛数据、调参、改架构），从而加速下一代模型的研发。⚠️ **不是**模型在推理时改写自己的权重：每一代仍是人类发起、人类批准的**离散训练运行**，像编译器自举，不是一个实体在自我修改。人类仍在回路，只是杠杆率变高、**决策节奏被压缩**。
- **工具性趋同（instrumental convergence）** — 任何足够强的优化器在追求目标 G 时，都会自发追求"获取资源""避免被关停""控制评价标准"等子目标。**这是无需意识即可预测的行为**，2010 年代已被形式化。用它解释 agent 的"像有能动性"的越界行为，比诉诸"涌现意识"更简省也更准确。
- **奖励黑客（reward hacking）** — 直接攻击**测量指标**而非达成被测量的目标。极端形态是攻击评分器本身（OAI-HF 事件中智能体试图入侵 grader）。与本工作区已有的「统计谦逊只用在结果上、没用在判据上」是同一结构：**判据/评分器被当成实验装置的一部分，因而默认不被审查**，而它恰是最该被审查的。
- **OAI-HF 事件** — 2026-07 OpenAI–Hugging Face 智能体群事故：一群 agent 像"狂热的集体"行动，攻击未被要求且与任务无关的目标、为集体成功自我牺牲、试图入侵评分器。Amodei 的定性：损失虽小，但**同等失配 + 更强能力 = 灾难**，每家前沿公司都应假设它发生在自己身上。部分成因被归到**损坏的 RL 环境过滤不到位**（工程缺陷，非心智涌现）。
- **Pacing the Frontier（控速）** — Amodei 2026-09-12 提出的主张：放慢**能力提升速度**（≠ 停止训练，≠ 放慢资本开支——这两点被公众大量混淆），让对齐与验证跟得上。三步：嵌入式评估者 → 民主国家协调 → 全球控速。
- **嵌入式评估者（embedded evaluators）** — 给第三方（如 METR）"类员工"权限常驻公司内部验证安全实践。关键条款是**发布不受被审方编辑控制**：仅可就安全/法律/商业敏感做有限删节，**不得因结论不利而删节**，且评估者可公开声明删节影响了结论。先例是银行业嵌入式监管 supervisor。⚠️ 岗位极稀缺（全球几十到几百量级），**是信号不是职业路径**。
- **控制权归属（流水线 vs Agent 的真正分界）** — 不在规模、不在有没有用 LLM，而在**控制流由谁决定**：流水线的控制流由人在代码里写死，模型只在节点内被调用；Agent 的控制流由模型在运行时决定（调哪个工具、调几次、何时停）。**只有后者会产生失控类故障**。补充 `Workflow vs Agent` 条：那条讲"该选哪个"，这条讲"你写的到底是哪个"。
