...
class LiteralScorer:
    ...
    # 同一条前沿滑动，换不来净收益"。**不成立** —— 阈值 0.15 同时拿到 89% 召回
    # 和 100% 空样本正确率，比这里的 0.2（84%）净高 5 个百分点。上一轮大概率
    # 只扫了 0.1 和 0.2，跳过了 0.15，于是把"我没扫到"当成了"不存在"。
    #
    # **那为什么还留着 0.2**：0.15 是在同一份 22 条 ground truth 上选出来的，
    # 用选点的数据去验证选点等于没验证；而那 5 个百分点只等于 1 条样本
    # （n=19，±1 条 ≈ ±5.3pp）。要动这个数，先得有一批没参与过选点的样本。
    # 完整前沿见 docs/rag-vs-literal.md §2。
    min_relevance = 0.2
