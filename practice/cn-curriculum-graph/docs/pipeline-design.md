...
- `CYCLE`（`validators/structure.py::check_no_cycles`，ERROR）会拒绝先修边成环 →
  同年级节点互为候选时，模型会对两个节点各点一次头，产出 A→B 又 B→A 的双向边，
  直接触发 CYCLE；剪枝规则据此断掉一个方向：**同年级只保留单向候选**，按
  `draft_id` 字典序，序在前的才能当序在后的前置（实现见
  `edges.py::candidate_prerequisites`）
...
**已知局限：按 draft_id 定方向是取舍，不是定论。** 这条规则解决的是"同年级
互为候选导致双向边"，不解决"方向选对了没有"——字典序在前的不一定是教学上
真正的前置，只是恰好排在前面。这个取舍还依赖一个更底层的前提：draft_id 的
字典序要大致等于抽取序。`pipeline/extract.py` 生成 `draft_id` 时把
`index`（同一 chunk 内第几个 draft）补零到两位（`f"{chunk.id}-{index:02d}"`），
就是为了维持这个前提——若不补零，一条条目抽出 ≥10 个知识点时，字典序会与
抽取序系统性反相关（`"...-10"` 在字符串比较下排到 `"...-2"` 之前），
"字典序 ≈ 抽取序"这个前提就系统性失效，同年级定方向这条规则会退化成
接近随机。也没有任何端到端测试真正验证"同年级互斥剪枝在完整流水线里生效"
（现有 fixture 里确实出现过同年级候选对，但没有用例让模型真的在两者之间选
方向）——这是已知的测试覆盖缺口，留给后续按需补。
