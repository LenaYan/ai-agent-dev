...
- **审查的三个盲区维度（逐任务 / 接缝 / 时间轴）** — 多 agent 开发里缺陷按"审查看不见的方向"分三类：**逐任务审查**只看单个任务内部，拦不住层与层的接缝；**接缝审查**看一次运行内各层是否协调，拦不住只在"跑完一次再跑一次"这条**时间轴**上出现的缺陷。实测分布：逐任务审查拦下的全是单层内部缺陷，中途接缝审查抓到 2 个 Critical 全在接缝，最终审查抓到的 Critical 只在重复运行时出现。**换审查单元比加密审查频次有效。**
- **形式完备而实质落空** — 本轮反复出现的同一病根的七种表现：恒真断言（`assert schema == schema`）、记账代码写在走不到的分支上、空产出静默绿灯、`RetryPolicy` 接不到真实故障、`timeout` 打不响、幂等去重反而吞掉真实记录、注释里的量化数字没验算过。共同点：**代码里挂着的东西比它实际做到的更像回事**，且**全部只有实跑能发现，读代码读不出来**。对策：要求审查者写一次性脚本实跑、对新加的断言做反向验证（故意改坏看它会不会红）、对"绿了"追问是不是因为测试根本区分不出来。
- **对比实验的失真方向** — 受控实验里最危险的不是编造数据，而是**合理默认的叠加**：故障注在层入口（最自然的写法）让重试代价恒为 0；fake 每类只挂一个判定器（最省事的写法）让红利占比虚高。两者都朝有利于被测框架的方向，且都不是有意为之。**实现者验证"我的脚本跑对了吗"，审查者必须问"这个设置测的是不是我以为的东西"** —— 后者只能由没参与设计的人来问。
