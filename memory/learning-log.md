...
## 2026-07-27（第三段）— MCP 暴露层：从"造数据"切到"用数据"，评测第一次给检索定价
...
- **下一步（断点清楚）**：
  1. **验收（唯一剩下的事，且必须人来做）**：重启 Claude Code 让 `.mcp.json` 生效，跑两段真实对话 —— 诊断「我家孩子说 0.45 比 0.405 小，因为 45 比 405 小。他到底卡在哪？」、规划「他要学会分数四则运算，按课标应该先补哪些？」。**验收判据不是能调通，是看 `misconceptions`/`evidence` 有没有真的进入 agent 的回答**，产出一段实录 + 诚实评价（"某字段其实没被用上"这种结论同样有价值）。
  2. 剩余 3 条未命中都指向同一件事：具体数字换掉后字面匹配就废（`20+5×2` vs 图里 `15+3×4`）。一个**未实测的假设**是把数字掩码成占位符再匹配 —— 按本项目纪律，动手前先花十分钟证伪。
