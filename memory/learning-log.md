...
- **一句话**：真装依赖、真下 `BAAI/bge-m3`、只替换 `_relevance()` 内部跑完受控对比 —— **向量没有赢**。同工作点（空样本正确率 = 100%）上 **字面 89%（阈值 0.15）vs 向量 79%（阈值 0.7）**，且**向量的命中集是字面命中集的真子集，零个独赢样本**。ADR-0006 不推翻，追加补记。产出 `practice/cn-curriculum-graph/docs/rag-vs-literal.md`，393 测试。
...
- **下一步**：
  ...
  3. ~~`MIN_RELEVANCE` 0.2 → 0.15 这个 5 个百分点的问题**要动就得先有一批没参与过选点的样本**~~ **[2026-07-28 已结案：真改了一次又改回来]** —— 改成 0.15 后 `test_match_misconceptions_returns_empty_when_nothing_is_close` 立刻变红："孩子不爱吃青菜"命中一条 correction_hint（"引导**孩子**对齐数位…"），相关度 **0.1667**，正好卡在 0.15 与 0.2 之间（7 字 = 6 个 bigram，只"孩子"一个命中，1/6）。**而评测在 0.15 上的空样本正确率仍报 100%** —— 因为 ground truth 里只有 3 条空样本，覆盖不到这个模式。**"前沿上 100%"和"这道门槛守得住"不是一回事，这次实测就是它们分开的地方。** 收益 1 条样本、代价一个被演示出来的误招模式，不划算。要再动，光有"没参与选点的样本"不够，还得有**足够多的空样本**压住这道门槛（现在 22 条里只有 3 条）。
